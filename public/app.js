...
function renderResearchResult(data){
  ...
  const saveChat=document.createElement('button');saveChat.type='button';saveChat.className='mini';saveChat.textContent='Add to current chat';saveChat.onclick=()=>{
    ...
    window.dispatchEvent(new CustomEvent('velora:save-to-chat',{detail:{user:$('#researchPrompt')?.value.trim()||'Web research',assistant:content,meta:`via Gemini + Google Search${data.mode==='deep'?' · Deep Research':''}`}}));
