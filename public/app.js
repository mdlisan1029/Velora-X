...
function isNearBottom(){
  ...
  return el.scrollTop + el.clientHeight >= el.scrollHeight - 180;
}
...
function addMessage(role,text,meta=''){
  ...
  if(shouldStick || role==='user') requestAnimationFrame(scrollBottom);
  return bubble;
}
...
function addHTMLActions(bubble){
  ...
  if(!/<html[\\s>]|<!doctype\\s+html/i.test(text)) return;
  const host=bubble.parentElement;
  if(host.querySelector('.html-actions')) return;
  ...
  dl.onclick=()=>downloadText(text,'velora-response.html');
  ...
  const wrap=document.createElement('span');wrap.className='html-actions';wrap.append(dl,preview);
  acts.appendChild(wrap);
}
function previewHTML(html){
  const w=window.open('about:blank','_blank');
  if(!w){alert('Popup blocked. Allow popups to preview HTML.');return;}
...
async function streamChat(){
  ...
  let full='',buf='',raf=0,pending=false;
  const render=()=>{
    raf=0;pending=false;
  ...
  try{
    ...
    while(true){
      ...
      for(const line of lines){
        ...
        const x=t.slice(5).trim();if(!x||x==='[DONE]')continue;
        try{
          const j=JSON.parse(x),delta=j.choices?.[0]?.delta?.content??'';
          if(delta){
            full+=delta;
            if(!pending){pending=true;raf=requestAnimationFrame(render);}
          }
        }catch{}
      }
    }
    if(raf)cancelAnimationFrame(raf);
    placeholder.textContent=full||'No text response returned.';
    ...
    if(stickToBottom) requestAnimationFrame(scrollBottom);
  }catch(err){
    if(raf)cancelAnimationFrame(raf);
    placeholder.textContent=`Error: ${err.message}`;placeholder.style.color='var(--bad)';history.pop();
  }finally{
    setBusy(false);promptEl.disabled=false;
    if(!window.matchMedia('(max-width:680px)').matches) promptEl.focus();
  }
}
