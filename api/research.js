...
async function synthesizeWithNara(query, results, mode, fallbackAnswer = '') {
  ...
  const upstream = await fetch(`${NARA_BASE}/chat/completions`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${key}`,
      'Content-Type': 'application/json',
...
function buildDeepQueries(query) {
  ...
  return [
    q,
    `${q} official primary sources`,
    `${q} latest developments analysis evidence`,
  ];
}
...
export default async function handler(req, res) {
  ...
  try {
    if (!process.env.TAVILY_API_KEY) {
      return res.status(503).json({ error: 'TAVILY_API_KEY is not configured in Vercel.' });
    }
    if (!process.env.NARA_API_KEY) {
      return res.status(503).json({ error: 'NARA_API_KEY is not configured in Vercel.' });
    }
    ...
    const model = DEFAULT_NARA_MODEL;
    ...
    if (mode === 'search') {
      ...
      const found = await Promise.all(planned.map((q) => tavilySearch(q, { advanced: true, domains })));
      queries = planned;
      tavilyCredits = found.reduce((sum, x) => sum + x.credits, 0);
      results = uniqueByUrl(found.flatMap((x) => x.results)).slice(0, 18);
    }

    if (!results.length && !tavilyAnswer) {
      return res.status(200).json({
        ok: true, mode, model, text: 'Tavily returned no web results for this query.', queries, sources: [], tavilyCredits,
      });
    }

    let text;
    try {
      text = await synthesizeWithNara(query, results, mode, tavilyAnswer);
    } catch (err) {
      if (mode === 'search' && tavilyAnswer) text = `${tavilyAnswer}\n\nNote: Nara synthesis was unavailable, so this answer uses Tavily's search summary.`;
      else throw err;
    }
    ...
    return res.status(200).json({
      ...
      sources,
      tavilyCredits,
      provider: `Tavily + NaraRouter (${model})`,
    });
  } catch (err) {
    return res.status(502).json({ error: err?.message || 'Web research service failed.' });
  }
}
