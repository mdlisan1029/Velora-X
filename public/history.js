(() => {
  ...
  async function getAllChats() {
    if (!state.db) return [];
    return requestIDB(state.db.transaction(STORE, 'readonly').objectStore(STORE).getAll());
  }

  async function putChat(chat) {
    if (!state.db || !chat?.id) return;
  ...
  function resetToWelcome() {
    ...
    if (clear) clear.click();
      ...
      messagesEl.innerHTML = '<div class="welcome"><div class="orb"></div><div class="eyebrow" style="margin-top:18px">VELORA X ONLINE</div><h2>Pick a model. Create.</h2><p>Use the provider and model selectors, then chat.</p></div>';
    }
  }
  ...
  function renderMessages(chat) {
    ...
    if (!chat?.messages?.length) {
      ...
      $$('.suggestion', messagesEl).forEach(b => b.addEventListener('click', () => {
        const p = $('#prompt');
        p.value = b.dataset.prompt;
    ...
    chat.messages.forEach(msg => {
      ...
      if (msg.role === 'assistant' && window.VeloraRender?.renderAssistantBubble) {
        window.VeloraRender.renderAssistantBubble(bubble, content);
        ...
        copy.onclick = () => window.VeloraRender.copyText ? window.VeloraRender.copyText(content) : navigator.clipboard?.writeText(content);
        const save = document.createElement('button');
        save.className = 'mini';
        save.type = 'button';
        save.textContent = 'Save .txt';
        save.onclick = () => {
          if (window.VeloraRender.downloadFile) {
            window.VeloraRender.downloadFile(content, 'velora-answer.txt', 'text/plain;charset=utf-8');
            return;
          }
          const blob = new Blob([content], {type:'text/plain;charset=utf-8'});
          const url = URL.createObjectURL(blob);
          const a = document.createElement('a');
          a.href = url; a.download = 'velora-answer.txt';
  ...
  function renderList(root) {
    ...
    if (!chats.length) {
      list.innerHTML = '<div class="vx-history-empty">No saved chats yet.</div>';
      return;
    }

    chats.forEach(chat => {
      const row = document.createElement('div');
      row.className = 'vx-history-item';

      const open = document.createElement('button');
      open.className = 'vx-history-open';
      open.type = 'button';
      ...
      rename.className = 'vx-history-icon';
      rename.type = 'button';
      rename.textContent = '✎';
      rename.title = 'Rename';
      rename.onclick = async (event) => {
        ...
        const name = prompt('Chat name', chat.title || 'New chat');
        if (!name?.trim()) return;
        chat.title = name.trim().slice(0,80);
        chat.updatedAt = Date.now();
        await putChat(chat);
        state.chats = await getAllChats();
        renderList(desktopCard);
        renderList(drawer);
      };

      const del = document.createElement('button');
      del.className = 'vx-history-icon';
      del.type = 'button';
      del.textContent = '×';
      del.title = 'Delete';
      del.onclick = async (event) => {
        ...
        const wasActive = state.activeId === chat.id;
        await deleteChat(chat.id);
        state.chats = await getAllChats();

        if (wasActive) {
          const fallback = [...state.chats].sort((a,b) => b.updatedAt - a.updatedAt)[0] || null;
          if (fallback) {
            saveActive(fallback.id);
            renderMessages(fallback);
          } else {
            // Do NOT immediately create another saved "New chat".
            saveActive(null);
            resetToWelcome();
          }
        }
  ...
  drawer.className = 'vx-history-drawer';
  drawer.innerHTML = '<div class="vx-history-head"><strong>Chat history</strong><button class="vx-history-icon" data-close type="button">×</button></div><button class="btn primary" data-new type="button" style="width:100%;margin-bottom:10px">+ New chat</button><div class="vx-history-list"></div>';

  const overlay = document.createElement('div');
  overlay.className = 'vx-history-overlay';
  ...
  $('[data-new]', drawer).onclick = () => {
    $('#newChatBtn')?.click();
    closeDrawer();
  };

  const desktopCard = document.createElement('section');
  desktopCard.className = 'card vx-history-card';
  ...
  if (topActions) {
    ...
    toggle.className = 'btn vx-history-toggle';
    toggle.textContent = 'History';
    toggle.type = 'button';
    toggle.onclick = openDrawer;
    topActions.prepend(toggle);
  }


  window.addEventListener('velora:save-to-chat', async (event) => {
    try {
      const detail = event?.detail || {};
      const user = String(detail.user || '').trim();
      const assistant = String(detail.assistant || '').trim();
      if (!assistant) return;
      const chat = await ensureActiveForChat();
      if (user) chat.messages.push({ role:'user', content:user });
      chat.messages.push({ role:'assistant', content:assistant, meta:String(detail.meta || 'via Gemini + Google Search') });
      chat.title = chat.title === 'New chat' && user ? titleFrom(user) : chat.title;
      chat.updatedAt = Date.now();
      await putChat(chat);
      state.chats = state.chats.filter(c => c.id !== chat.id).concat(chat);
      renderList(desktopCard);
      renderList(drawer);
      const chatTab = $('.tab[data-pane="chat"]');
      if (chatTab && !chatTab.classList.contains('active')) chatTab.click();
      renderMessages(chat);
    } catch {}
  });

  async function ensureActiveForChat() {
    if (state.activeId) {
      const existing = state.chats.find(c => c.id === state.activeId);
  ...
  async function init() {
    ...
    state.activeId = savedActive && state.chats.some(c => c.id === savedActive) ? savedActive : null;

    if (!state.chats.length) {
      // First visit: create exactly one blank chat.
      const first = createChatRecord();
      saveActive(first.id);
      state.chats = [first];
      await putChat(first);
    } else if (!state.activeId) {
      const newest = [...state.chats].sort((a,b) => b.updatedAt - a.updatedAt)[0];
      saveActive(newest?.id || null);
    }
    ...
    if (active?.messages?.length) renderMessages(active);
    ...
    window.fetch = async (input, init = {}) => {
      ...
      try {
        ...
        isChat = new URL(url, location.href).pathname === '/api/chat';
      } catch {}
      ...
      const activeChat = await ensureActiveForChat();
        ...
        clone.text().then(async raw => {
          ...
          if (raw.startsWith('data:') || raw.includes('\ndata:')) {
            for (const event of raw.split(/\r?\n\r?\n/)) {
              for (const line of event.split(/\r?\n/)) {
                ...
                try {
                  ...
                  assistant += String(j.choices?.[0]?.delta?.content ?? j.choices?.[0]?.message?.content ?? '');
                } catch {}
              }
            }
          } else {
            try {
              ...
              assistant = String(j.data?.choices?.[0]?.message?.content ?? j.choices?.[0]?.message?.content ?? '');
            } catch {}
          }

          const sent = body.messages?.findLast?.(m => m.role === 'user')
          ...
          chat.messages = [
            ...(body.messages || []).map(m => ({ role:m.role, content:String(m.content ?? '') })),
            {
              role:'assistant',
              content:assistant,
              meta: response.headers.get('X-Velora-Provider') ? `via ${response.headers.get('X-Velora-Provider')}` : ''
            }
          ];
