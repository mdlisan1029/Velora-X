...
function normalize(p, index = 0) {
  ...
  const id = String(p.id || name.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || `provider-${index + 1}`);
...
export function getProviders() {
  const builtIns = [
    ...
    provider('OpenAI', 'OPENAI', { baseUrl: 'https://api.openai.com/v1', model: 'gpt-5-mini', imageModel: 'gpt-image-1', website: 'https://platform.openai.com' }),
