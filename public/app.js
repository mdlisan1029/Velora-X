...
function isNearBottom(){return messagesEl.scrollTop + messagesEl.clientHeight >= messagesEl.scrollHeight - 120;}
...
async function streamChat(){
  ...
  try{
    ...
    while(true){const {value,done}=await reader.read();if(done)break;buf+=decoder.decode(value,{stream:true});const lines=buf.split(/\r?\n/);buf=lines.pop()||'';for(const line of lines){const t=line.trim();if(!t.startsWith('data:'))continue;const x=t.slice(5).trim();if(!x||x==='[DONE]')continue;try{const j=JSON.parse(x);const delta=j.choices?.[0]?.delta?.content??'';if(delta){const wasNearBottom=isNearBottom();full+=delta;placeholder.textContent=full;if(wasNearBottom)scrollBottom()}}catch{}}}
