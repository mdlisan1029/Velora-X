...
function buildRequestBody(body, provider, model) {
  ...
  if (Number.isInteger(body.max_tokens) && body.max_tokens > 0) out.max_tokens = Math.min(body.max_tokens, 16000);
...
function uniqueModels(provider, requestedModel) {
  ...
  if (provider.model) candidates.push(String(provider.model));
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!configured.length) return res.status(503).json({ error: 'No providers configured. Add API keys in Vercel Environment Variables.' });
      ...
      for (const model of models) {
        ...
        try {
          ...
          if (!upstream.ok) {
            ...
            // Try the next model for Gemini (or the next provider generally) on transient/model-access errors.
            if (RETRYABLE.has(upstream.status) || upstream.status === 401 || upstream.status === 403 || upstream.status === 404 || upstream.status === 400) {
              continue;
            }
            continue;
          }
          ...
          if (stream && upstream.body) {
            ...
            try {
              ...
              if (!res.writableEnded) res.write(`data: ${JSON.stringify({ error: 'Upstream stream timed out or closed unexpectedly.' })}\n\n`);
            } finally {
          ...
          clearTimeout(timer);
          return res.status(200).json({ provider: provider.name, providerId: provider.id, model: data.model || model, data, fallbackCount: errors.length });
        } catch (err) {
          errors.push({ provider: provider.name, model, status: 0, message: err.name === 'AbortError' ? 'Request timed out' : err.message });
          continue;
        } finally {
          clearTimeout(timer);
        }
    ...
    return res.status(503).json({ error: 'All configured AI providers failed or are unavailable.', ...(expose ? { providers: errors } : {}) });
  } catch (err) {
